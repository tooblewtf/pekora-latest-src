...
do
	...
	function Util.CalculateRelativeDimensions(guiObject, guiDims, mockup_dims, rootGuiSize)
		...
		if Util.ShouldUseVRAppLobby() then
			guiResolution = rootGuiSize
		end
	...
	function Util.ShouldUseVRAppLobby()
		local UserInputService = game:GetService('UserInputService')
		local result = Util.IsFastFlagEnabled('EnabledAppShell3D') and UserInputService.VREnabled
		return result
	end
